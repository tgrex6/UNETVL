...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int], lstm: bool = True, no_kan: bool = True):
    valid = []
    for trained_model in model_dict:
        ...
        expected_output_folder = get_output_folder(dataset_name_or_id, trained_model['trainer'], trained_model['plans'],
                                                   trained_model['configuration'], fold=None, lstm=lstm, no_kan=no_kan)
...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
    ...
    predict_command = ''
    trained_model_folder = get_output_folder(dataset_name_or_id, trainer_name, plans_identifier, configuration_name, fold=None, lstm=lstm, no_kan=no_kan)
    ...
    if 'previous_stage' in plans_manager.available_configurations:
        ...
        predict_command += generate_inference_command(dataset_name_or_id, prev_stage, plans_identifier, trainer_name,
                                                      folds, None, output_folder='OUTPUT_FOLDER_PREV_STAGE', lstm=lstm,no_kan=no_kan) + '\n'
...
def find_best_configuration(dataset_name_or_id,
    ...
    allowed_trained_models = filter_available_models(deepcopy(allowed_trained_models), dataset_name_or_id, lstm, no_kan)

    for m in allowed_trained_models:
        output_folder = get_output_folder(dataset_name_or_id, m['trainer'], m['plans'], m['configuration'], fold=None, lstm=lstm, no_kan=no_kan)
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                output_folder_1 = get_output_folder(dataset_name_or_id, m1['trainer'], m1['plans'], m1['configuration'], fold=None, lstm=lstm, no_kan=no_kan)
                output_folder_2 = get_output_folder(dataset_name_or_id, m2['trainer'], m2['plans'], m2['configuration'], fold=None, lstm=lstm, no_kan=no_kan)
    ...
    if best_key.startswith('ensemble___'):
        ...
        print(best_key)
        tr, name, pl, c = convert_identifier_to_trainer_plans_config(best_key)
        return_dict['best_model_or_ensemble']['selected_model_or_models'].append(
            {
                'configuration': c,
                'trainer': tr,
    ...
    # print it
    print_inference_instructions(return_dict, instructions_file=join(nnUNet_results, dataset_name, 'inference_instructions.txt'), lstm=lstm, no_kan=no_kan)
    return return_dict


def print_inference_instructions(inference_info_dict: dict, instructions_file: str = None, lstm: bool = True, no_kan: bool = True):
    ...
    for j, i in enumerate(inference_info_dict['best_model_or_ensemble']['selected_model_or_models']):
        ...
        _print_and_maybe_write_to_file(generate_inference_command(dataset_name_or_id, c, pl, tr, inference_info_dict['folds'],
                                         save_npz=is_ensemble, output_folder=output_folder_name, lstm=lstm, no_kan=no_kan))
...
def find_best_configuration_entry_point():
    ...
    parser.add_argument('--no_overwrite', action='store_true',
                        help='If set we will not overwrite already ensembled files etc. May speed up concecutive '
                             ...
                             'outdated results.')
    parser.add_argument('--lstm', type=str2bool, default=True, required=True,
                        help="Use this to specify if the model uses LSTM or not. Default: True")
    parser.add_argument('--no_kan', type=str2bool, default=True, required=True,
                        help="Use this to specify if the model uses KAN or not. Default: True")
    ...
    find_best_configuration(dataset_name, model_dict, allow_ensembling=not args.disable_ensembling,
                            ...
                            strict=False, lstm=args.lstm, no_kan=args.no_kan)
...
def accumulate_crossval_results_entry_point():
    ...
    parser.add_argument('--no_kan', type=str2bool, default=True, required=True,
                        help="Use this to specify if the model uses KAN or not. Default: True")
    args = parser.parse_args()
    trained_model_folder = get_output_folder(args.dataset_name_or_id, args.tr, args.p, args.c, args.lstm, args.no_kan)
