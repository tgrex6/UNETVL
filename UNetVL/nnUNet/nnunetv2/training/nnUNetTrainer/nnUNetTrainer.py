...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), lstm: bool = True, no_kan: bool = True):
    ...
    def on_epoch_end(self):
        ...
        # if self._best_mean_fg_dice is None or self.logger.my_fantastic_logging['mean_fg_dice'][-1] > self._best_mean_fg_dice:
        #     self._best_mean_fg_dice = self.logger.my_fantastic_logging['mean_fg_dice'][-1]
        #     self.print_to_log_file(f"Yayy! New best mean FG Dice: {np.round(self._best_mean_fg_dice, decimals=4)}")
        #     self.save_checkpoint(join(self.output_folder, 'checkpoint_best_mean.pth'))
