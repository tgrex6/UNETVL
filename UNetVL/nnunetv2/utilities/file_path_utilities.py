...
def convert_trainer_plans_config_to_identifier(trainer_name, plans_identifier, configuration, lstm, no_kan):
    if lstm:
        name = 'UNETR_LSTM'
        if no_kan:
            name += '_no_kan'
    ...
    return f'{trainer_name}__{name}__{plans_identifier}__{configuration}'
