...
class SequenceConv3d(nn.Conv3d):
    ...
    def forward(self, x):
        assert x.ndim == 3  # Expect batch, sequence, channel dimensions
        if self.seqlens is None:
...
class VitPosEmbed3d(nn.Module):
    def __init__(self, seqlens, dim: int, allow_interpolation: bool = True):
        super().__init__()
        self.seqlens = seqlens  # Expecting a three-dimensional sequence length (D, H, W)
        self.dim = dim
        self.allow_interpolation = allow_interpolation
        self.embed = nn.Parameter(torch.zeros(1, *seqlens, dim))
        self.reset_parameters()
    ...
    @property
    def _expected_x_ndim(self):
        # This will always return 5: batch, channel, depth, height, width
        return len(self.seqlens) + 2
    ...
    def forward(self, x):
        ...
        if x.shape[1:] != self.embed.shape[1:]:
            ...
            embed = interpolate_sincos(embed=self.embed, seqlens=x.shape[2:-1], mode="trilinear")  # Using trilinear for 3D
        else:
            embed = self.embed
        return x + embed
